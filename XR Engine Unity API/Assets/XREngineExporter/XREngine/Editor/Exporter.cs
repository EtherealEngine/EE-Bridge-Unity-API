...
namespace XREngine
{
	public class Exporter : EditorWindow
	{
		...
		[MenuItem("XREngine/GLTF Exporter")]
		static void Init()
		{
			RefreshLogo();
		...
		static void RefreshLogo()
		{
			if (logo == null)
			{
				logo = (Texture2D)AssetDatabase.LoadAssetAtPath("Assets/XREngineExporter/XREngine/Editor/xrengine_black_icon.png", typeof(Texture2D));
			}
		}
		...
		void OnGUI()
		{
			...
			else
			{
				...
				{
					var grid = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/XREngineExporter/XREngine/Alignment/AlignmentPlane.prefab", typeof(GameObject));
					var obj = GameObject.Instantiate(grid);

					obj.name = "XREngine Alignment Grid";
				}
			}
			...
			if (GUILayout.Button("Reset Exporter Defaults"))
			{
				...
				XREngineExporterOptions.ExportAnimations = true;
			}*/
		...
		private static void ShowAnimationModifiersUI()
		{
			...
			XREngineExporterOptions.ExportAnimations = EditorGUILayout.Toggle("Export Animations", XREngineExporterOptions.ExportAnimations);
		...
		public static Tuple<string, string, string> GenerateGLTFZipped(GameObject selectedObject)
		{
			...
			XREngineExporterOptions.CompressDirectory(path.Item2, path.Item1 + "/" + path.Item3);
			XREngineExporterOptions.DeleteDirectory(path.Item2);
		...
		public static Tuple<string, string, string> GenerateGLTF(GameObject selectedObject)
		{
			...
			string fullpath = EditorUtility.SaveFilePanel("glTF Export Path", XREngineExporterOptions.LastEditorPath, selectionName, "zip");
			XREngineExporterOptions.LastEditorPath = Path.GetDirectoryName(fullpath);

			string selectedName = Path.GetFileNameWithoutExtension(fullpath);

			var path = XREngineExporterOptions.LastEditorPath;
			if (!string.IsNullOrEmpty(path))
			{
				...
				if (info.Exists)
				{
					if (XREngineExporterOptions.ExportAnimations == true)
					{
